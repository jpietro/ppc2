...
<div>
  ...
  <h4 v-show="isEmailFilled" class="user_h1_email" >
    O email deve ser no formato email@gmail.com</h4>
  ...
  <div>
    ...
    <h4 v-show="isAgeOk" class="user_h1_idade" >
      A idade tem que estar entre 18 e 99, e deve ser apenas numérica</h4>
...
export default {
  ...
  computed: {
    ...
    isFilled() {
      const {
        isAgeValid, isEmailLog, isSoEmpty, email, age, system,
      } = this;
  ...
  methods: {
    ...
    isAgeValid(number) {
      ...
      return (number.length > 0 && ((number > 17 || number < 99) || this.hasOnlyNumber(number)));
    },
    isEmailValid(string) {
      const regex = /\S+@\S+\.\S+/;
      return !(regex.test(string));
    ...
    isEmailLog(string) {
      const emailLog = `email_${this.numberUser}@gmail.com`;
      if (string === emailLog) {
        return false;
      }
      return true;
